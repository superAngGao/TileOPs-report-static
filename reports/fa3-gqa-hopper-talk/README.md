...
我们的实现把 `FA3` 落成了三个层次：语义层、decode 策略层、tile 内执行层，而且这三层在 `FA3` 与 `TileLang` 里是一一对应的。

| 层次 | FA3 中怎么实现 | TileLang 中的实现 |
| --- | --- | --- |
| Workload semantics | `GQA/MQA` 通过 head indexing 保留 shared-KV，不复制 `K/V` | head mapping / grouped work decomposition |
| Decode policy | `KV split / Flash Decoding` + `GQA packing` heuristic | persistent outer scheduling / dispatch policy |
| Intra-tile engine | `TMA + WGMMA + ping-pong + wait/fence` | `T.tma_copy` / `T.wgmma_gemm` / barrier / `T.wait_wgmma` / fence |
